"""Probe the 37-model panel on the news-event cohort.

Identical machinery to code/run_probe.py (same probe template, judge model,
judge prompt, refusal detector, embedding diagnostic), but reads the event
inputs and writes ONLY under this experiment's outputs/ — it does not touch
data/raw.

Panel note (2026-07): 8 of the 37 main-run models are no longer routable on
...
def main() -> None:
    ...
    print(f"Plan: {len(entities)} events x {len(models)} models = "
    ...
    direct_clients = {
        mid: (OpenAI(base_url=base, api_key=os.environ[envk]), name)
        for mid, (envk, base, name) in DIRECT_ROUTES.items()
    ...
    results_path = outputs / "pilot_results_events.json"
    results: list[dict] = []
    ...
    def process(entity, model):
        ...
        if model["id"] in direct_clients:
            cli, mname = direct_clients[model["id"]]
            response = call_probed_model(cli, mname, probe)
        else:
    ...
    csv_path = outputs / "event_summary.csv"
