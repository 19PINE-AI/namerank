...
sys.path.insert(0, str(T6 / "scripts"))
# judge_v2: context-aware grader that gives zero credit to context echo
from importlib.machinery import SourceFileLoader  # noqa: E402
_v2 = SourceFileLoader("run_probe_v2",
                       str(T6 / "scripts" / "04_run_probe_v2.py")).load_module()
judge_v2 = _v2.judge_v2
...
def main() -> None:
    ...
    canon = json.loads((out / "canonical_golds_norm.json").read_text())
    judge_tpl = (T6 / "inputs" / "judge_prompt.txt").read_text()   # judge v2
    records = json.loads((out / "pilot_results_tiers.json").read_text())
    ctx_of = {eid: e["context"] for eid, e in entities.items()}
    ...
    def rejudge(rec, gold_text):
        if rec["is_refusal"]:
            return dict(rec, coverage=0.0, accuracy=0.0, score=0.0,
                        rationale="refusal")
        s = judge_v2(client, judge_tpl, rec["entity_name"],
                     ctx_of[rec["entity_id"]], gold_text, rec["response"])
    ...
    ckpt = out / "_rejudge_v2_ckpt.json"
    done_pairs = {}
    if ckpt.exists():
    ...
    (out / "pilot_results_tiers_v2.json").write_text(
    ...
    with open(out / "tier_per_entity_v2.csv", "w", encoding="utf-8") as f:
    ...
    if bojie_gold:
        ...
        (out / "bojie_convergence_v2.json").write_text(json.dumps(conv, indent=1,
                                                    ensure_ascii=False))
