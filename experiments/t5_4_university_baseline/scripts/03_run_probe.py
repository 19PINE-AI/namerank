"""Probe the per-university baseline cohort on the 36-model event panel.

Identical machinery to the t4_1 event run (same probe template, judge, refusal
detector, embedding diagnostic, and the same 36-model panel: the 28 main-run
survivors plus the 8 IKP-v2 replacement models).  Reads this experiment's
inputs and writes only under its outputs/.

Usage: python 04_run_probe.py [--parallel 12] [--max-entities N]
...
# Same dead-model set as the t4_1 event run (2026-07 provider churn; grok's
# direct-API partial coverage was discarded there and it stays excluded here).
DEAD_MODELS = {
...
def main() -> None:
    ...
    entities = json.loads((inputs / "univ_entities.json").read_text())
    gold = json.loads((inputs / "univ_gold.json").read_text())
    ...
    results_path = outputs / "pilot_results_univ.json"
    results: list[dict] = []
    ...
    csv_path = outputs / "univ_summary.csv"
