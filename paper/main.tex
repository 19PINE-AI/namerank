...
To manipulate the artifact mention directly, we re-probed all $11$ pairs under a controlled intervention: arm~A uses a role-only disambiguating context for the creator; arm~B appends one clause naming the artifact (e.g., ``creator of Tianshou''), holding everything else fixed. Figure~\ref{fig:inversion}b reports the per-pair lift (full table in Appendix~\ref{app:asymmetry}). Injection raises creator NameRank exactly where the observational analysis predicts: most for creators the panel barely stores, not at all for senior leaders whose recognition is already saturated; and by model tier the lift grows as capability decreases, so naming the artifact is load-bearing precisely for models that have not independently stored the creator.
...
Gold length, context leakage, or Wikipedia presence drive the results & Dedicated ablations and regressions & All three rejected & App.~\ref{app:confounds} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Floors are calibrated and confounds rejected.} Thirty fictional entities establish the noise floor: near-zero for dense golds, elevated for boilerplate golds---and floor-adjusting \emph{widens} the credential gap (Appendix~\ref{app:synthetic}). Dedicated ablations reject gold-answer length, context leakage, and Wikipedia presence as alternative explanations; in particular, NameRank is not a has-a-Wikipedia-page flag---the binary flag explains ${\sim}2\%$ of long-tail variance against ${\sim}40\%$ for $h$-index (Appendix~\ref{app:confounds}).
