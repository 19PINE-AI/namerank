...
% ══════════════════════════════════════════════════════════════
\begin{abstract}
What a frontier model says about a person or tool is often the first description a human sees, so whether a name has propagated into training corpora is a measurement problem. Citations explain about a third of the variance in whether a model recognizes a researcher \citep{li2026ikp}; we treat the residual as the target and introduce \textbf{NameRank}, a continuous $[0,1]$ recognition score. We probe $5{,}719$ entities in $54$ cohorts with one open-ended question across $37$ models; an independent LLM judge scores each response against a curated gold answer on coverage$\times$accuracy, so fluent hallucinations earn nothing ($211{,}603$ records). Six findings: \textbf{(i)~a credential treadmill}---seven of nine Olympic-style credentials score at or below a working-researcher baseline, while the marquee prizes (Nobel, Turing, Fields) invert this, accruing for decades; \textbf{(ii)~artifact over creator}---in $8$ of $11$ verified pairs the tool out-ranks its maker; \textbf{(iii)~$h$-index carries the bibliometric signal}---citations add nothing once it is known, yet ${\sim}60\%$ of variance is unexplained; \textbf{(iv)~a corpus-density gradient}---Stanford CS faculty score twice Tsinghua's, part of an English-corpus ladder across institutions and countries; \textbf{(v)~external calibration}---on $258$ news events with pageview ledgers, recognition loads on peak salience, not persistence; \textbf{(vi)~self-report is not measurement}---asked to rank their own recognition, models recover the panel ordering only partially ($\rho \approx 0.5$--$0.6$), though they rarely claim a fictional name. The metric is silent below threshold rather than misleading; findings survive paraphrase, cutoff, cross-judge, and confound controls. We release all probes, gold answers, responses, and code.
\end{abstract}
\begin{center}
\small
