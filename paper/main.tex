...
\centering
\small
\caption{Selected cross-domain entities on a single NameRank axis, spanning the universal, discriminative, and silent zones. Diagnostic reference-set entities (Altman, Hinton, Karpathy, and below) carry deliberately high-detail gold answers, which cap coverage for any $150$-word response---levels are gold-conditional (Section~\ref{sec:results-events}, Appendix~\ref{app:synthetic}), so even universally known figures sit near $0.6$ here. Because the score is graded against a gold answer, an entity's level depends on how much its gold demands: the same model responses score higher against a short gold and lower against a dense one. Appendix~\ref{app:medal-tiers} shows this directly on one person probed three ways, and shows that a length-normalized gold with a context-aware judge removes the sensitivity.}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/fig_awards.pdf}
    \caption{Mid/late-career awards as recognition predictors ($n = 576$ probed on the $36$-model event panel, with in-run OpenAlex and IMO controls). \textbf{(a)}~The extended ladder: the three marquee late-career awards (green) sit $+0.15$ to $+0.20$ above the working-researcher baseline (dashed), inverting the credential treadmill, while mid-career honors (orange) and the IMO early credential (hatched) sit at or below it; bars are cohort means with $\pm 1$ SEM, and each label carries the baseline-relative delta. \textbf{(b)}~The lifetime signal: NameRank standardized within each marquee award and binned by years since the award---recognition rises from $-0.5\sigma$ in the first decade to $+0.25\sigma$ two-to-four decades later. Full tables in Appendix~\ref{app:awards}.}
    \label{fig:awards}
\end{figure}

The credential treadmill covers \emph{early} credentials---competition medals and PhD fellowships awarded near the start of a career. A natural question is whether the decay is a property of credentials in general or of their timing. We therefore added an award-ladder cohort spanning the career arc: three marquee late-career prizes (Turing Award, all $77$ Wikidata-listed laureates; Fields Medal, $64$; Nobel Prize in Physics, $64$ from $2000$--$2023$), three mid-career honors (G\"odel Prize $80$, ACM Prize in Computing $18$, ACM Fellow and MacArthur Fellowship $60$ each), and an early-career fellowship (Sloan Research Fellowship, $49$). All are probed and judged by the unchanged pipeline, on the same $36$-model panel as the news-event cohort (Section~\ref{sec:results-events}); crucially, the comparison is anchored by \emph{in-run} controls whose contexts and gold answers are copied verbatim from the main run---$60$ OpenAlex working researchers (the same baseline) and $40$ IMO golds---so the ladder is immune to the cross-fleet vintage drift (Section~\ref{sec:results-validation}). Construction and the full table are in Appendix~\ref{app:awards}.

\textbf{The three marquee awards invert the treadmill} (Figure~\ref{fig:awards}a). Nobel Physics, Turing, and Fields laureates score $0.695$, $0.674$, and $0.640$---$+0.15$ to $+0.20$ \emph{above} the working-researcher baseline, and the only individual-credential cohorts anywhere in the study to clear it by more than the ICPC/Putnam margin. The mid-career honors behave like the early credentials: ACM Fellow ($0.473$), G\"odel Prize ($0.467$), and MacArthur ($0.443$) all sit at or below the baseline, and IMO gold ($0.386$) below them. The lead is not a gold-density artifact---the marquee laureates carry dense Wikipedia-intro golds, which \emph{cap} coverage (Section~\ref{sec:results-events}), so their standing is if anything understated: restricted to Wikipedia-gold members alone, Nobel/Turing/Fields still sit at $0.65$--$0.70$ (Appendix~\ref{app:awards}).

\textbf{What separates the marquee tier is name-propagation, not prestige or timing.} The pattern is exactly what the corpus-presence account predicts. A Nobel, Turing, or Fields award is a \emph{singular, uniquely-named, heavily-reported event}---each laureate acquires a dense, cleanly-attributed documentary footprint (news coverage, encyclopedic biographies, decades of ``Turing laureate X'' references) on top of a career's worth of named production. The honors that fail are the diluted ones: ACM Fellow elevates over a thousand people, its per-name reproduction is low; MacArthur spans all fields, so its computer-science-legible footprint is thin; and the G\"odel Prize is a \emph{paper} award, so its recognition attaches to the cited paper rather than binding to a single name---the same artifact-over-creator asymmetry documented in Section~\ref{sec:results-mechanism}. Consistent with this, a regression on the $140$ ladder members with a confident OpenAlex bibliometric match finds that a marquee-award indicator adds $R^2 = 0.19$ of NameRank variance \emph{beyond} $\log_{10}(h\text{-index})$, and $+0.16$ in level at matched $h$-index---the award marks recognition the bibliometrics miss (Appendix~\ref{app:awards}; the $h$-match is imperfect for laureates, so we read this as directional). The Sloan Fellowship's position just above the baseline ($0.519$) is not a genuine early-career exception: its roster is a Wikidata-coverage sample biased toward the fellows notable enough to have an article, and it selects into English-corpus-dense US research careers---an upper bound, flagged accordingly in Appendix~\ref{app:awards}.

\textbf{A lifetime clock, made visible.} Within each marquee award, standardizing recognition against the award's own mean and pooling across the three isolates the effect of time since the award (Figure~\ref{fig:awards}b): laureates in their first post-award decade sit $0.5$ standard deviations \emph{below} their cohort, rising to $+0.2$ to $+0.25\sigma$ by two-to-four decades out. Recognition keeps accumulating for roughly thirty years after even the most prestigious award is conferred---direct evidence that NameRank reads a cumulative documentary ledger rather than current-term salience (Section~\ref{sec:discussion-clock}).
...
Two further utility sites from the same university ecosystem, probed on the $36$-model award panel (Section~\ref{sec:results-awards}), sharpen the mechanism into a within-genre contrast. \emph{Hackergame}, USTC's annual public capture-the-flag competition, scores $0.459$---well above both \emph{icourse.club}, USTC's student course-review community ($0.211$), and tuixue.online on the same panel ($0.182$). All three have large, engaged user bases; what separates them is documentary attribution. Hackergame has a unique, non-compositional name and, after each edition, publishes official and community write-ups as named English-readable repositories on GitHub---exactly the indexable, name-bearing production the credential cohorts lack---whereas the course-review and visa-monitoring sites are referenced functionally in Chinese-language chatter with no durable named footprint. Reach is comparable; recognition tracks the write-ups.

\subsection{Self-Reported Recognition: Asking a Model Is Not Measuring It}
\label{sec:results-selfreport}

An obvious shortcut would collapse the instrument to a single prompt: instead of probing and verifying, ask a frontier model which names it recognizes. We test whether the shortcut works---and, underneath it, whether models have introspective access to the quantity NameRank measures behaviorally. Three panel models (GPT-5.5, Claude Opus 4.6, Gemini 3.1 Pro, at main-run settings) each judged the same $3{,}350$ name pairs over $430$ entities ($400$ stratified across the NameRank range plus the $30$ fictional controls of Appendix~\ref{app:synthetic}), stating which of the two they know more about, that they know both equally, or that they recognize neither; a tie-extended Bradley--Terry model \citep{bradley1952rank,davidson1970ties} converts each model's verdicts into a self-reported ranking. Design, estimation, and full tables are in Appendix~\ref{app:selfreport}.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig_selfreport.pdf}
\caption{Self-reported recognition (three frontier panel models, $3{,}350$ shared pairs). \textbf{(a)}~Bradley--Terry self-ranking against panel NameRank (GPT-5.5 shown; orange markers are decile means): $\rho = 0.53$--$0.61$ across the three models. \textbf{(b)}~What the self-ranking tracks: agreement with the \emph{other vendors'} self-rankings ($\rho \approx 0.90$) and with the prevalence component of NameRank---the share of the $37$-model panel that knows the entity ($\rho = 0.72$--$0.76$)---is far higher than with the reporting model's \emph{own} behavioral scores among entities it demonstrably knows ($|\rho| \le 0.09$). \textbf{(c)}~Verdict shares (pooled over models) on boundary pairs whose one side the model behaviorally knows, split by whether corpus fame agrees with the model's own knowledge, and on fictional-trap pairs: when fame contradicts its own knowledge the model abstains half the time, and the fictional side is claimed in $0.1\%$ of trap pairs.}
\label{fig:selfreport}
\end{figure}

The shortcut half-works, and the failing half is diagnostic (Figure~\ref{fig:selfreport}). Self-reported rankings recover the panel ordering only partially---a usable prior, not a substitute for measurement. More telling is \emph{what} self-report reads (panel~b): each model's self-ranking tracks how \emph{widely} a name is known across the panel, is essentially uncorrelated with the model's own behavioral record among entities it demonstrably knows, and agrees with the other two vendors' self-rankings far more closely than with the model's own scores. When its own knowledge contradicts corpus fame (panel~c), the model mostly abstains rather than reading its own state. Introspection, in short, returns a shared corpus prior, not a reading of the model's own weights \citep{kadavath2022language,yin2023selfaware}.

In one direction, however, the claims are precise, echoing the silent-rather-than-misleading property of Section~\ref{sec:scope}: opposite a name the model demonstrably knows, the fictional side of a trap pair was preferred exactly once in $684$ presentations, and fictional-vs-fictional pairs drew ``neither'' almost without exception. Self-reported recognition errs by under-claiming, not fabrication: trustworthy when asserted, insufficient as a measurement.
