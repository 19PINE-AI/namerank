...
The credential ladder above measures \emph{gold} medalists only, so it cannot say whether the medal's grade carries signal. To test this we probed the complete medal roster of a single contest --- NOI 2009: every gold, silver, and bronze medalist ($103$ in all) --- with the pipeline otherwise unchanged. The roster comes from the official CCF medal list, cross-validated line-by-line against the community OIerDb database (identical names, schools, and scores); the released files include each medalist's contest score and rank. The twenty golds re-enter with their main-run probes and gold answers verbatim; the new silver and bronze entities receive gold answers built from the same boilerplate recipe with only the tier band changed, so gold length and density are matched across tiers and the short-boilerplate synthetic-null floor (Appendix~\ref{app:synthetic}) applies equally to all three. Probe names follow the main run's romanization conventions and carry the Chinese characters, which --- together with the year-and-school context --- disambiguate medalists whose Pinyin collides with common names. The run uses the $36$-model 2026 panel of the news-event cohort (Appendix~\ref{app:events}); the $63$ entities shared with the released main run reproduce their released ordering almost exactly (Spearman $\rho = 0.915$, mean shift $-0.006$).

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig_medal_tiers.pdf}
\caption{The complete NOI 2009 medal roster. \textbf{(a)}~Per-medalist NameRank by tier (dots), with tier means and bootstrap $95\%$ CIs, against the synthetic-null floor for this gold recipe. \textbf{(b)}~NameRank against the contest score that determined the tiers, with within-tier least-squares fits. Callouts mark the medalists discussed in the text.}
...
\textbf{The ladder is a cliff.} Gold and non-gold are separated by a large, unambiguous gap, while silver and bronze are statistically indistinguishable (Table~\ref{tab:medal-tiers}). Every gold medalist clears the synthetic-null floor; the silver and bronze cohort \emph{means} sit at or below it. Read against Property~2 of the metric, the silver and bronze rosters are not weakly recognized --- as cohorts they are silent, with an individually recognized tail. The mechanism matches the corpus-density account of Section~\ref{sec:results-credentials}: the ${\sim}20$-person national gold roster is the documented one --- admissions coverage, competitive-programming lore, team-selection reporting --- while silver and bronze rosters barely mint name-bearing English text at all.

\textbf{The cut matters; marginal points do not.} Because tier is a threshold function of contest score, a pooled regression of NameRank on score looks respectable, but nearly all of it is the gold/non-gold cut: within tiers the explained variance collapses (Table~\ref{tab:medal-tiers}, Figure~\ref{fig:medal-tiers}b). Recognition does not accrue per point; it accrues on crossing the line into the documented roster.

\textbf{The recognized tail is the treadmill again.} The silver and bronze medalists who do clear the floor are exactly the ones whose \emph{post-medal} careers produce named artifacts: the top-scoring silver medalist beats the gold-tier \emph{mean} on the strength of a prominent computer-vision research career (Fan Haoqiang), and the top bronze medalist ranks near the bottom of the contest's score column yet seventh of $103$ in recognition (Li Bojie, a systems researcher and entrepreneur --- and the author of this paper, included with the same boilerplate gold as every other medalist). Within tiers, the medal-year cohort's gender gap also disappears ($0.173$ vs.\ $0.164$ for non-gold men and women; the 2009 gold roster has no women), consistent with the controlled comparison of Appendix~\ref{app:gender}.

\textbf{The score is graded against a gold answer, so one person can carry several scores.} One medalist in this roster also appears in the main run as two other probe entities, and the three scores differ threefold: $0.35$ when graded against the two-sentence medalist gold, $0.41$ against a fellowship-cohort gold, and $0.12$ against the dense $150$-word diagnostic biography ($0.09$ in the released run, the value shown in Table~\ref{tab:headline-placement}). The underlying model responses are equally correct in all three cases; what differs is how much the gold answer demands --- a model that recognizes the person reproduces the short medalist gold in full, while even a detailed correct response covers only part of the long biography. NameRank levels are therefore comparable only between entities graded against same-style gold answers (the within-person analogue of the Appendix~\ref{app:synthetic} floors), which is why every medalist in the tier ladder above received an identically structured gold.
