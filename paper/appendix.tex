...
The actual probe template used in the Chinese sub-run is the Mandarin translation of the English template, with the literal Chinese characters supplied to the model. We render it here in pinyin (with English translation in parentheses) because the default pdf\LaTeX{} build does not include CJK font support; the literal Chinese version is in the released artifact, and the document can be rebuilt with XeLaTeX to display the characters directly.
...
Cohort-mean NameRank is stable across judges (e.g.\ \texttt{gpt5\_system\_card\_author} $0.087$/$0.165$/$0.123$ under Gemini/Claude/GPT-5; \texttt{long\_tail\_researcher\_openalex} $0.669$/$0.630$/$0.631$), and the silent$\to$discriminative$\to$universal ladder is preserved under all three. The headline findings are not a single-judge artifact.
