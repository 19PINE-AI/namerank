...
Table~\ref{tab:full-cohort} lists all $54$ cohorts with sample size, mean NameRank, refusal rate, and inclusion criteria. Rows are keyed by the cohort identifiers used in the released dataset; the body text refers to the same cohorts by readable names.
...
reference\_pilot & 37 & 0.452 & 15\% & Hand-curated diagnostic reference set \\
...
long\_tail\_researcher\_ikp & 159 & 0.158 & 47\% & Researcher subset from \citet[\S5.7]{li2026ikp} \\
...
\begin{figure}[H]
    ...
    \caption{Per-model panel statistics, sorted by mean score (graphical form of Table~\ref{tab:per-model}). Left panel: mean per-record score across $5{,}719$ entities, colored by vendor class. Right panel: refusal rate, aligned to the same model axis. Two failure styles are visible: the strict cluster at the bottom (gpt-oss-20b-think, mistral-small-24b, qwen3-32b/8b-think) refuses heavily, while the fluent-hallucinator cluster (ernie, llama-4-maverick, gemma-3-4b, phi-4, ministral-3b, llama-3.2-1b) shows near-zero refusal with widely varying mean scores. The panel mean integrates over both styles.}
...
\midrule
NOI China gold & 0.404 \\
ICPC World Finals gold & 0.381 \\
CMO China gold & 0.359 \\
OpenAlex long-tail researchers & 0.356 \\
IMO gold & 0.333 \\
Putnam fellows & 0.331 \\
$\cdots$ & $\cdots$ \\
Books & 0.157 \\
Podcasts & 0.152 \\
Named methods & 0.144 \\
conference & 0.131 \\
GPT-5 system-card authors & 0.126 \\
Online courses & 0.113 \\
\bottomrule
\end{tabular}
\end{center}
